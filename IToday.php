...
<body>
	...
	<div style="padding-top: 3%; padding-bottom: 2%; padding-right: 5%; padding-left: 5%">
		...
		<table class="table">
		  	...
		  	<?php
				$sql = "SELECT lease.Lease_Id,property.Name,tenant.Fname From property inner join lease on property.Property_Id = lease.PropertyId inner join tenant on lease.TenantId = tenant.Tenant_Id where TIMESTAMPDIFF(day,CURDATE(),IncDate) = 0 and Lstatus = 'Ongoing'";
				$result = $conn->query($sql);
				if($result->num_rows == 0)
				{
					echo '<script>alert("There are no leases due for increment")</script>';
		  ...
		  <tbody>
		    <tr>
		      ...
		      <td><button type="button" name = "view"class="btn btn-dark"><a href="RenewLease.php?action=view&l_id=<?php echo $row['Lease_Id']?>" style="color:white;">Renew</a></button></td>
