...
require('db.php');
$sql="SELECT lease.Unit, lease.RenewalDate, lease.IncDate, lease.Lease_Id,property.Name,tenant.Fname, tenant.Email From property inner join lease on property.Property_Id = lease.PropertyId inner join tenant on lease.TenantId = tenant.Tenant_Id where TIMESTAMPDIFF(day,CURDATE(),EndDate) Between 32 and 91 and Lstatus = 'Ongoing'";
